...
def process_tags(orderby="count", desc="desc", limit=None):
    ...
    for result in results:
        output = "{:<5} | {:<32.31} | {:<5}".format(count, result[0], result[1])
        print(output)
...
def tags_bar_graph(tag_count, tag_names):
    ...
    fig.update_layout(title="Tags bar graph")
    fig.show()
...
def process_authors(orderby="count", desc="desc", limit_authors=None):
    ...
    authors_index = {}
    count = 1
    print("{:<5} | {:<31.30} | {:<3} | {:<50.48} \n".format("Index", "Author", "Count", "URL"))
    for result in results:
        output = "{:<5} | {:<31.30} | {:<3} | {:<50.48}".format(count, result[0], result[1], result[2])
        print(output)
...
def process_comments(limit=None):
    ...
    if limit is not None:
        ...
        print("{:<5} | {:<17} | {:<5} \n".format("Index", "Month, Year", "Total Comments"))
        for row in range(len(dates)):
            output = "{:<5} | {:<17} | {:<5}".format(count, dates[row], total_comments_per_month[row])
            print(output)
            month_year_split = dates[row].split()
            sql = "SELECT Blogs.Title, Authors.FullName, Blogs.Date, Blogs.Comments, Blogs.CompleteURL FROM Blogs " \
...
def process_images():
    ...
    for result in sorted_blogs:
        print("{:<33.32} | {:<5}".format(result[0], result[1]))
    conn.close()
...
def most_recent(limit=10):
    ...
    if limit != 10:
        ...
        execute = cur.execute(statement)
        results = execute.fetchall()
        print("{:<5} |{:<30.28} | {:<25.25} | {:<20.20} | {:<40.37} | {:<25.25} | {:<40.30}".format("Index",
                                                                                    "Title",
                                                                                    "Author",
                                                                                    "Date Published",
                                                                                    "Description",
        ...
        for each_result in results:
            print("{:<5} |{:<30.28} | {:<25.25} | {:<20.20} | {:<40.37} | {:<25.25} | {:<40.30}".format(count,
                                                                                                      each_result[0],
                                                                                                      each_result[1],
                                                                                                      each_result[2],
                                                                                                      each_result[3],
        ...
        if tags_commands[0] in response_list:
            ...
            while tags_input != "no":
                ...
                if tags_input == "no":
                    ...
                    try:
                        int(tags_input)
                        print("{:<5} | {:<40.40} | {:<20}".format("Index", "Blog Title", "Blog URL"))
                        if int(tags_input) in command_processed[2].keys():
                            count = 1
                            for each_value in command_processed[2][int(tags_input)]:
                                output = "{:<5} | {:<40.40} | {:<20}".format(count, each_value[0], each_value[1])
                                count += 1
                                print(output)
                        else:
            ...
            while authors_input != "no":
                ...
                if authors_input == "no":
                    ...
                    print("{:<5} | {:<30.29} | {:<40.38} | {:<20.20} | {:<70.70}".format("Index",
                                                                             "Title",
                                                                             "Description",
                                                                             "Date",
                                                                             "Blog URL"))
                    try:
                        ...
                        if int(authors_input) in command_processed[3].keys():
                            ...
                            for each_value in command_processed[3][int(authors_input)]:
                                output = "{:<5} | {:<30.29} | {:<40.38} | {:<20.20} | {:<70.70}".format(count,
                                                                                                     each_value[1],
                                                                                                     each_value[2],
                                                                                                     each_value[3],
                                                                                                     each_value[4])
            ...
            while comments_input != "no":
                comments_input = input(
                    "Would you like to view the articles within a month? Type the number or type 'no' to "
                    "return to main display: ")
                if comments_input == "no":
                    break
                else:
                    print("{:<5} | {:<40.38} | {:<25.25} | {:<17.17} | {:<8} | {:<70.70}".format("Index",
                                                                                              "Title",
                                                                                              "Author",
                                                                                              "Date",
                                                                                              "Comments",
                    ...
                    try:
                        ...
                        if int(comments_input) in command_processed[2].keys():
                            ...
                            for each_value in command_processed[2][int(comments_input)]:
                                output = "{:<5} | {:<40.38} | {:<25.25} | {:<17.17} | {:<8} | {:<70.70}"\
